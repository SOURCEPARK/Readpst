...
int         mode_A       = 0;   // new mode for command line option -N
...
int main(int argc, char* const* argv) {
    ...
    while ((c = getopt(argc, argv, "a:bC:c:Dd:emNhj:kMo:qrSt:uVwL:8"))!= -1) {
        switch (c) {
        case 'N': //wie e nur mode_MH ist wie bei S -> es wird auch write_separate_attachment() in write_normal_email() durchgeführt
            mode = MODE_SEPARATE;
            mode_A   = 1;
            mode_MH  = 0;
            mode_EX  = 1;
...
void write_separate_attachment(char f_name[], pst_item_attach* attach, int attach_num, pst_file* pst)
{
    ...
    int x = 0;
    char *temp = NULL;
    char *attachdir = malloc(strlen(f_name)+5);
    sprintf(attachdir, ".%s", f_name);
    ...
    if (!attach_filename) {
        ...
        if(!(mode_A))
            ...
            sprintf(temp, "attach%i", attach_num);  //Attachment Name ohne Präfix
    } else {
        ...
        do {
            ...
            if (x == 0)
                if(!(mode_A))
                    ...
                    sprintf(temp, "%s", attach_filename); //Attachment Name ohne Präfix
            else
                if(!(mode_A))
                    sprintf(temp, "%s-%s-%i", f_name, attach_filename, x);
                else
                    sprintf(temp, "%s-%i", attach_filename, x); //Attachment Name ohne Präfix
    ...
    if((mode_A)){
        mkdir(attachdir, 0777); //no checking if folder is already existing because mkdir() is not destroying content of old folder if already existing (besides that very unlikely that one would already exist)
        if(chdir(attachdir)){ //Change to directory for attachments of current E-Mail
            int err = errno;
            DEBUG_WARN(("write_separate_attachment: Cannot go into directory (%s): %s\n", attachdir, strerror(err)));
        }
    }
    if (!(fp = fopen(temp, "w"))) {
        ...
        fclose(fp);
        if((mode_A)){
            if(chdir("..")){  //After finishing writing of attachments going back in the actual working directory
                int err = errno;
                DEBUG_WARN(("write_separate_attachment: Cannot go up directory (..): %s\n", strerror(err)));
            }
        }
    }
    if (temp)       free(temp);
    if(attachdir)   free(attachdir);
    DEBUG_RET();
}
        ...
        for (attach = item->attach; attach; attach = attach->next) {
            ...
            if (attach->method == PST_ATTACH_EMBEDDED) {
                ...
                if (acceptable_ext(attach)) {
                    ...
                    if(mode == MODE_SEPARATE && (mode_A)) //Write inline attachment although separate attachments are already written
                        write_inline_attachment(f_output, attach, boundary, pst);
                }
            }
        }
